...
namespace Remalux.AR
{
    ...
    public class WallDetectionManager : MonoBehaviour
    {
        ...
        public const float WALL_HEIGHT = 3.0f;          // Увеличиваем стандартную высоту стены до 3 метров
        ...
        private void CalculateWallDimensions(ARPlane plane, out Vector3 position, out float width, out float height)
        {
            ...
            // Всегда используем фиксированную высоту стены вместо вычисления
            height = WALL_HEIGHT;
        }
        ...
        private void CreateWall(ARPlane plane)
        {
            ...
            // Добавляем BoxCollider для лучшего взаимодействия
            var boxCollider = wall.AddComponent<BoxCollider>();
            boxCollider.size = new Vector3(targetWidth, targetHeight - WALL_BOTTOM_OFFSET, 0.05f);
            boxCollider.center = new Vector3(0, (targetHeight + WALL_BOTTOM_OFFSET) / 2, 0);

            // Устанавливаем слой и тег
            wall.layer = 10; // AR Layer
            wall.tag = "Wall";
