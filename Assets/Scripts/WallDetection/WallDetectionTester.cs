...
namespace Remalux.WallDetection
{
    public class WallDetectionTester : MonoBehaviour
    {
        ...
        // Add public properties needed by TestSceneGenerator
        ...
        // Fix the CreateTexture method to use modern texture creation
        private Texture2D CreateTexture(int width, int height, TextureFormat format)
        {
            // Use correct texture creation
            return new Texture2D(width, height, format, false, false);
        }
        
        // Add a method to process test images for TestSceneGenerator
        public void ProcessTestImage(Texture2D originalImage, Texture2D maskTexture)
        {
            if (originalImage == null || maskTexture == null)
                return;
                
            // Create result texture if needed
            if (resultTexture == null || resultTexture.width != originalImage.width || resultTexture.height != originalImage.height)
            {
                if (resultTexture != null)
                    Destroy(resultTexture);
                resultTexture = CreateTexture(originalImage.width, originalImage.height, TextureFormat.RGBA32);
            }
            ...
            {
                if (maskPixels[i].a > 0.5f) // If mask is visible at this pixel
                {
                    // Apply some highlighting effect
                    resultPixels[i] = Color.Lerp(originalPixels[i], maskPixels[i], 0.7f);
                }
                else
                {
        ...
        // Add analysis methods needed by TestSceneGenerator
        public float GetWallCoverage()
        {
            ...
            return (float)wallPixels / pixels.Length;
        }
